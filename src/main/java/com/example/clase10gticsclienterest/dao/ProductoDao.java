...
public class ProductoDao {
    ...
    public List<Product> listar() {
        ...
        String endPoint = "http://localhost:8080/product"; //url a consumir

        ResponseEntity<Product[]> responseEntity = restTemplate.getForEntity(endPoint, Product[].class);

        if(responseEntity.getStatusCode().is2xxSuccessful()){
            Product[] body = responseEntity.getBody(); //se recupera la información obtenida
    ...
    public void guardar(Product product){
        ...
        HttpEntity<Product> httpEntity = new HttpEntity<>(product,httpHeaders); //Para ajuntar el objeto a enviar

        if(product.getId() ==null){
            restTemplate.postForEntity(endPoint,httpEntity,Product.class); //envía el objeto
        }else{
            restTemplate.put(endPoint,httpEntity,Product.class); //actualiza el objeto
        }
